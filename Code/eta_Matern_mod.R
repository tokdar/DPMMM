eta_Matern_mod <-function(A,A_star,B,B_star,Omega, zeta, sigma2, K.Matern, ell_prior, gamma, m_gamma, sigma2_gamma){
  ...
  for(i in 1:nRep){
    ...
    Omega.inv_i <- diag(1/Omega[i,1], t.T)
    kappa_i <- kappa[i,]
    z_i <- kappa[i,] / Omega[i,1]
    ...
    C.tilde.inv <- solve(C[[ell_index[i]]][[k]]) + Omega_i
    R.tilde <- chol(C.tilde.inv)
    m.tilde <- backsolve(R.tilde, backsolve(R.tilde, kappa[i,] + m_gamma[k]*c(C.inv[[ell_index[i]]][k]%*%rep(1,t.T)), transpose = TRUE))
