...
gen_smooth_mplxd <- function(lambdaA,lambdaB=NULL ,num.ctrials,start.time=0, end.time=1000, bw=(end.time-start.time)){
  ...
  for (i in 1:num.ctrials){
    ...
    #jit = sample(1:200,1)
    # Azeem modification
    #jit = sample(1:sin.period, 1)
    # do not randomly shift, shift by proportions of period
    jit = i*sin.period/num.ctrials
    ...
    lambda = ((sin((t+jit)/sin.period*2*pi)+1)/2*(max_rate-min_rate))+min_rate 
